...
void saveErrors(const std::string& trace_dir, const Eigen::aligned_vector<double>& acc_errs, 
const Eigen::aligned_vector<double>& gyro_errs, const Eigen::aligned_vector<double>& gp_errs, 
...
int main(int argc, char** argv)
{
    ...
    {
        img_names.push_back(frames[i].name);
    }
    std::vector<size_t> sorting_idxs = argSort(img_names);
    Eigen::aligned_vector<Frame> frames_sorted;
    for(size_t i = 0; i < sorting_idxs.size(); i++)
    {
        size_t idx = sorting_idxs[i];
        frames_sorted.push_back(frames[idx]);
    }
    frames.clear();
    for(size_t i = 0; i < frames_sorted.size(); i++)
    {
        frames.push_back(frames_sorted[i]);
    }

    // ToDo: load camera poses and then transform to body.
    // this avoid to have to run the python script that does it.
    Eigen::aligned_vector<Sophus::SO3d> init_knots_q_gb;
    Eigen::aligned_vector<Eigen::Vector3d> init_knots_r_gb;
    /* We do not need to use init_knots_ts. 
    The reason is that the spline assumes that the time of the first knot is 0.0 and uses 
    the var: start_t_ns_, to account for the "actual" time of the first knot. 
    In our case, it is the time of the first frame estimated by Colmap.
    */
    ...
    double t_cam_gp_init;
    if (loadScalar<double>(alignment_dir + "/spline_globalsensor_time_offset.txt", t_cam_gp_init))
    {
        std::cout << "Loaded spline global sensor time offset: " << std::setprecision(15) << t_cam_gp_init << "\n\n";
    }
    else
    {
        GVI_FUSION_ASSERT_STREAM(false, "Could not load spline global sensor time offset");
    }
    ...
    "Increase bias error dt such that bias_err_dt_s >= order * dt_bias_spline_s.");*/
    ...
    std::cout<< "Initial gravity in colmap frame:\n"<< spline_wb.getGravity() << "\n\n";
    ...
    saveErrors(trace_dir, init_acc_errors, init_gyro_errors, init_gp_errors, init_reproj_errors, false);
    ...
    double t_offset_gs_imu = spline_wb.getSplineImuTimeOffset() - spline_wb.getSplineGlobSensTimeOffset();
    std::cout<< "Optimized global sensor - imu time offset: "<< t_offset_gs_imu << " [s]\n\n";
    ...
    std::cout<< "Gravity in colmap frame:\n"<< spline_wb.getGravity() << "\n\n";
    ...
    saveErrors(trace_dir, opti_acc_errors, opti_gyro_errors, opti_gp_errors, opti_reproj_errors, true);
